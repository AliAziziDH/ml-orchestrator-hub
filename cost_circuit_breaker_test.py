...
class TokenCostLedger(BaseModel):
    ...
    output_tokens: int = Field(default=0, description="Total output tokens generated across LLM calls")
    accumulated_cost_usd: float = Field(default=0.0, description="Accumulated financial cost in USD")
    budget_limit_usd: float = Field(default=5.0, description="Max budget cap allowed before circuit-breaking")
    is_budget_exhausted: bool = Field(default=False, description="Flag indicating if the budget limit is breached")

class TrainingTelemetry(BaseModel):
    current_fold: int = Field(default=0, description="The CV fold currently undergoing active training")
    total_folds: int = Field(default=10, description="Total CV folds planned for the training run")
    fold_scores: List[float] = Field(default_factory=list, description="Computed metric scores per completed fold")
    progress_percentage: float = Field(default=0.0, description="Completion percentage of the active training task")
    last_heartbeat: Optional[str] = Field(None, description="ISO timestamp of the last heartbeat pulse")
    stall_rounds: int = Field(default=0, description="Number of rounds stalled with same output/error (oscillation index)")
    last_error_signature: Optional[str] = Field(None, description="Last recorded traceback/error signature")

class ExperimentMeta(BaseModel):
    experiment_id: str = Field(..., description="Unique experiment identifier, e.g., EXP-HP-001")
    experiment_tag: str = Field(..., description="Short tag descriptive of the run (e.g., baseline_slsqp)")
    model_architecture: str = Field(..., description="Detailed description of model ensemble pipeline")
    oof_cv_score: Optional[float] = Field(None, description="Computed global Out-Of-Fold Cross-Validation score")
...
def evaluate_yield_point(state: Phase4AgentState, epsilon: float = 1e-4) -> str:
    ...
    if cost >= budget_limit:
        ...
        state["error_message"] = f"CRITICAL: Budget limit of ${budget_limit} exhausted. Current cost: ${cost:.4f}."
        return "antigravity_recovery"
        
    # 2. State Oscillation / Idle Loop Check (Stall Rounds Breaker)
    if stall_rounds >= 3:
        state["circuit_breaker_triggered"] = True
        state["error_message"] = f"CRITICAL: State oscillation detected. Jules stalled for {stall_rounds} rounds with the same footprint."
        return "antigravity_recovery"
    ...
    if attempts > 2 and len(fold_scores) >= 2:
        ...
        if delta <= epsilon:
            state["circuit_breaker_triggered"] = True
            state["error_message"] = f"CRITICAL: Stagnation detected. Metric delta ({delta:.6f}) <= epsilon ({epsilon})."
            return "antigravity_recovery"
            
    # 4. Hard Max Attempts Check
    if attempts >= 5:
        state["circuit_breaker_triggered"] = True
        state["error_message"] = f"CRITICAL: Max attempts ({attempts}) exceeded without convergence."
        return "antigravity_recovery"
