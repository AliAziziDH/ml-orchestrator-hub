...
def run_slsqp_simulation():
    ...
    state = {
        "messages": [
            ...
            {
                "role": "user",
                "content": "Conductor: Start EXP-HP-001. Find optimal convex weights for XGBoost + CatBoost OOF ensemble using SLSQP.",
            },
        ],
    ...
    state["messages"].append(
        {
            "role": "assistant",
            "content": "Jules: Executing data_loader_tool for house-prices-kaggle fold 1-10.",
        }
    )
    ...
    for attempt in range(1, 4):
        state["messages"].append(
            {"role": "user", "content": f"Compiler/Test: Attempt {attempt} failed. {error_msg}"}
        )
        state["messages"].append(
            {
                "role": "assistant",
    ...
    print(
        f"-> Total Accumulated Tokens: {state['cost_tracker']['input_tokens'] + state['cost_tracker']['output_tokens']}"
    )
    ...
    print(
        f"-> Has Yield Point been reached? {yield_point_triggered} (Stall Rounds: {stall_rounds})"
    )
    ...
    recovery_query = (
        "Resolve fold 10 dimension mismatch, crop array to 1459, and run SLSQP convex optimizer."
    )
    print(f'-> Recovery Query: "{recovery_query}"')

    # Retrieve tools semantically for the EVALUATION stage
    corrective_tools = tool_finder.select_tools_semantically(
        recovery_query, active_stage=state["stage"]
    )
    ...
    state["experiment"]["key_insights"] = (
        f"Optimal weights: XGB={optimal_weights['XGBoost']:.2f}, Cat={optimal_weights['CatBoost']:.2f}. Dimension issue solved."
    )

    deploy_tools = tool_finder.select_tools_semantically(
        "Sync experiment to ledger", active_stage=state["stage"]
    )
