...
class ContextTokenMonitor:
    ...
    def audit_llm_call(self, model_name: str, prompt_tokens: int, completion_tokens: int) -> TokenCostLedger:
        """
    ...
    def check_compaction_trigger(self, current_context_tokens: int, max_context_limit: int) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "compaction_level": "URGENT" if utilization_ratio >= 0.85 else "PROACTIVE" if should_compact else "NONE",
            "timestamp": datetime.now(timezone.utc).isoformat()
        }

    def log_suspend_resume_overhead(self, suspend_ckpt_tokens: int, resume_input_tokens: int) -> Dict[str, Any]:
        """
